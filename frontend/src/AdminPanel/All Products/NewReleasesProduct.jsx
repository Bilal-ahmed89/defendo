...
import { useAddNewReleasesDetailsMutation } from '../../features/newReleases';
import * as Yup from 'yup';
import CloudinaryWidget from '../CloudinaryWidget';


function NewReleasesProduct() {

    const [addNewReleasesDetails, { error }] = useAddNewReleasesDetailsMutation();
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        try {
            await addNewReleasesDetails(newProduct);
            resetForm();
            setColors([]);
            setVariants([]);
            setUploadedImages([]);
                ...
                <div className="row">
                    <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
                        {({ errors, touched, values }) => (
                            <Form className="form">
                                <h1 className="h3 mb-3 fw-normal text-center text-uppercase heading">Add Product <strong>New Release</strong></h1>
...
export default NewReleasesProduct;
