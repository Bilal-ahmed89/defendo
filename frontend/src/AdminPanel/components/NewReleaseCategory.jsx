...
import {useGetNewReleasesDetailsQuery, useDeleteNewReleasesDetailsMutation} from '../../features/newReleases'
import NewReleasesProduct from '../All Products/NewReleasesProduct';


const NewReleasesCategory = () => {

    const [Products, setProducts] = useState([]);
    const { data } = useGetNewReleasesDetailsQuery();
    const [ deleteNewReleasesDetailsMutation ] = useDeleteNewReleasesDetailsMutation();
    const [isAddProductPopupOpen, setAddProductPopupOpen] = useState(false);


    const handleDelete = async (id) => {
        try {
            await deleteNewReleasesDetailsMutation(id);
    ...
    return (
        <div className='p-2 container'>
            <h2 className='text-center mt-4'>New Relases Category</h2>
